...
- **VIP 기준/기사관리 계정별 이동** (완료, DB 마이그레이션 필요 — `0013_driver_owner_scope.sql`):
  - VIP 고객 기준을 계정별 개별 설정으로 전환. user1~5는 설정 화면에서 자기 고객 규모에 맞는 기준을
    각자 설정하고, admin은 더 이상 이 값을 직접 관리하지 않음(admin 전체 합산 뷰는 기본값 사용).
  - 기사(drivers)에 `owner_username`을 추가해 계정별 소유로 전환. user1~5는 설정 화면에서 자신의
    기사만 등록/조회/수정하고, admin은 전체 계정의 기사를 계정별로 모아서 조회하며 등록 시 담당 계정을
    지정. 배정 이력이 없는 기사는 admin이 완전히 삭제할 수 있고, 이력이 있으면 삭제 대신 비활성화만
    가능(주문 정산 이력 보존).
  - 배송관리에서 기사 배정 시 보여주는 기사 목록도 주문과 동일한 계정 범위로 scoping(같은 계정의
    기사만 배정 가능, admin은 전체).
  - 엑셀 업로드 결과 카드/이력 표에 "총 주문(서로 다른 주문번호 수)"과 "처리건수(엑셀 행 수)"의
    차이를 설명하는 문구 추가.
- **정산관리 계정별 이동 + 기사 정산 화면** (완료): 정산관리를 admin 전용에서 계정별 기능으로 전환.
  user1~5는 자신이 등록한 기사의 정산만 보고, admin은 계정 필터(전체/특정 계정)로 조회. 기사 로그인
