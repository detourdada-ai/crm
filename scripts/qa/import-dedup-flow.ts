...
async function main() {
  ...
  try {
    ...
    record("QA-1a. Analyze 결과 — 신규 1건, 이미등록 0건", text.includes("신규 주문") && /신규 주문[\s\S]{0,20}1건/.test(text));
    ...
    text = await reviewText(page);
    record("QA-3a. 누적 업로드 — 신규 1건 + 이미등록 1건으로 구분", /신규 주문[\s\S]{0,20}1건/.test(text) && text.includes("이미 등록된 주문"));
    await confirmRegister(page);
    ...
    record("QA-9a. 다른 tenant(user3)의 동일 정보 주문은 신규로 분류(중복/후보 아님)", /신규 주문[\s\S]{0,20}1건/.test(text) && !text.includes("⚠️"));
    await confirmRegister(page);
    ...
    await page.getByText("엑셀 분석 완료").waitFor({ state: "visible", timeout: 15000 });
    text = await reviewText(page);
    const wasNewAtAnalyze = /신규 주문[\s\S]{0,20}1건/.test(text);
