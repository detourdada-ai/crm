...
export default async function DriverPage({ searchParams }: { searchParams: Promise<{ date?: string }> }) {
  ...
  const [{ orders, itemSummaries }, shift] = await Promise.all([listMyDeliveriesAction(date), getMyShiftAction()]);

  return (
    <div className="space-y-6">
      <PageHeader title="내 배송" description="배정된 배송 목록입니다. 완료 후 배송완료 버튼을 눌러주세요." />

      <MyDeliveriesList key={date} orders={orders} itemSummaries={itemSummaries} initialShift={shift} selectedDate={date} />
    </div>
  );
}
